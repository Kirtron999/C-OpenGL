...
static GLfloat rComponent = 0, bComponent = 0, gComponent = 0;
static GLfloat fLowLight[4] = { rComponent, gComponent, bComponent, 1 };
static GLfloat fLowLightT[4] = { rComponent, gComponent, bComponent, 1 };
static GLenum FogMode = GL_LINEAR;



typedef struct {                     // Рамка для контейнера ссылок    GLTVector3 vLocation;
	GLTVector3 vUp;
	GLTVector3 vForward;
...
void SetupRC()
{
	...
	{
		gltInitFrame(&teapots[iSphere]);    // Инициализируется камера        
											// Выбираются случайные положения между -20 и 20 с шагом 0.1
	...
	{
		gltInitFrame(&cubes[iSphere]);    // Инициализируется камера        
										  // Выбираются случайные положения между -20 и 20 с шагом 0.1
...
{
	glClearColor(rComponent, gComponent, bComponent, 1.0f);
	int i;
	...
	fLowLightT[0] = rComponent;
	fLowLightT[1] = gComponent;
	fLowLightT[2] = bComponent;
	glEnable(GL_FOG);		// Включается туман
	glFogfv(GL_FOG_COLOR, fLowLightT);
	// Цвет тумана соответствует фону

	glFogf(GL_FOG_START, 2.0f);	//	Насколько далеко
								//	начинается туман
	glFogf(GL_FOG_END, 15.0f);	//	Насколько далеко
								//	заканчивается туман
	glFogi(GL_FOG_MODE, FogMode); //	Какое уравнение 
									//	тумана используется
	...
	// Рисуем случайным образом расположенные чайники Юта
	...
	// Рисуем случайно расположенные кубы
	...
	glColor3f(1, 1, 1);
	// Все рисуется в каркасном виде
	glPolygonMode(GL_FRONT_AND_BACK, GL_LINE); //ПРи изменении на GL_FILL остаются внутренние окружности тора
...
void SubMenuFogCheck(int choice) {
	switch (choice) {
		case 0:
			FogMode = GL_LINEAR;
			break;
...
void MenuInit() {
	...
	int subMenuFog = glutCreateMenu(SubMenuFogCheck);
	int subMenuColor = glutCreateMenu(SubMenuColorCheck);

	glutSetMenu(subMenuFog);
	glutAddMenuEntry("GL_LINEAR", 0);
	glutAddMenuEntry("GL_EXP", 1);
	glutAddMenuEntry("GL_EXP2", 2);
	...
	glutAddMenuEntry("Серый", 6);
	glutAddMenuEntry("Черный", 7);

	glutSetMenu(menu);
	glutAddSubMenu("Режим тумана", subMenuFog);
	glutAddSubMenu("Цвет тумана", subMenuColor);
...
int main(int argc, char* argv[])
{
	...
	glutCreateWindow("OpenGL NotOnlySphereWorld With Fog");
